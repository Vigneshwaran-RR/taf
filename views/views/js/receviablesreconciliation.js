...
function initTable(TableDataColumns, TableDataRows, TableDIV, searchInputDIV) {
  ...
  voucherTable = $('#voucher_table').dataTable( {
            ...
            "nRow": TableDataRows,
            "iStart": 0,
            "iEnd": TableDataRows.length,
            "aiDisplay": "hello world",
           /*  "aoColumnDefs": [
              {
                "mData": null,
...
{
    var giCount = 1;
    $('#voucher_table').dataTable().fnAddData( [
        giCount+".1",
        giCount+".2",
        giCount+".3",
        giCount+".4",
...
/* function getDetailsOfAllDeals()
{
    var request = $.ajax({
      url: "Voucher/getVouchersForConsolidation",
      type: "POST",
      data: {},
      dataType: "html",
    });
...
function getVouchersForDeal (dealID,functiond,sDate,eDate) {
  ...
  StartDateFieldValue = $("#startdatefield").val();
  // alert(StartDateFieldValue);
  var EndDateFieldValue = "";
  EndDateFieldValue = $("#enddatefield").val();
  // alert(EndDateFieldValue);
  var request = $.ajax({
    url: "Voucher/getVouchersForConsolidation",
    type: "POST",
    data: {orderstatus : order_status ,paymentmethod : payment_method ,orderReceivableStatus : OrderReceivableStatus ,startDate : StartDateFieldValue ,endDate : EndDateFieldValue},
    dataType: "html",
  });
  ...
  request.done(function(data) {
    ...
    if(grandSummary) {
        ...
        $("#totalorderamount").empty();
        $("#totalsettled").empty();
        $("#totaldifference").empty();
        $("#totalcredit").empty();
        $("#totalsuspence").empty();
        // $("#paymentNotes").empty();
        $("#totalorderamount").append(grandSummary['totalorderamount']);
        $("#totalsettled").append(grandSummary['totalsettled']);
        $("#totaldifference").append(grandSummary['totaldifference']);
        $("#totalcredit").append(grandSummary['totalcredit']);
        $("#totalsuspence").append(grandSummary['totalsuspence']);
...
$(document).ready(function(){
  ...
  // Added date format text inside textbox
  ...
  $("#changeRecievableStatus_btn").click(function(){
    var paymentStatus = $("#changeRecievableStatus").val();
    setStatus(paymentStatus);
  });

  // Get details of all deals
  // getDetailsOfAllDeals();
...
{
  var request = $.ajax({
    url: "Order/setOrderStatusForOrderID",
    type: "POST",
